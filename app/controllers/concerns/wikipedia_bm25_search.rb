...
module WikipediaBM25Search

  extend ActiveSupport::Concern
  include Elasticsearch::DSL

  # type = quick, search, facet
  def wikipedia_bm25_search(repo, type, query, filters, page, sort)
          ...
          must do
            if query.present?
              multi_match do
                query     query
                type      "best_fields"
                fields    ["title^3", "text_field", "opening_text^2"]
                # fuzziness "AUTO"
              end
            else
              match_all
    ...
    logger.debug "-------------".yellow
    if took_ms < 1000
      logger.debug "WikipediaBM25 ES #{type.titleize} Query: " +
