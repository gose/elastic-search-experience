...
module ATMSearch
  ...
  def atm_search(repo, type, query, filters, page, sort)

    filter_lookup = {}
    filter_lookup['Amount'] = 'amount'
    filter_lookup['Operation'] = 'operation'
    filter_lookup['Success'] = 'success'
    filter_lookup['City'] = 'atm_city'
    filter_lookup['Network'] = 'atm_network'
    filter_lookup['Plan'] = 'customer_plan'
    filter_lookup['Card Type'] = 'card_type'
...
=end
        aggregation :operation do
          terms do
            field filter_lookup['Operation']
            size  5
          end
        end
        aggregation :success do
          terms do
            field filter_lookup['Success']
            size  5
          end
        end
        aggregation :city do
        ...
        aggregation :network do
          terms do
            field filter_lookup['Network']
            size  5
          end
        end
        aggregation :plan do
          terms do
            field filter_lookup['Plan']
            size  5
          end
        end
        aggregation :card_type do
          terms do
            field filter_lookup['Card Type']
            size  5
          end
        end
      end
    ...
    if took_ms < 1000
      logger.debug "ATM ES #{type.titleize} Query: " +
